...
class Roster {
  ...
  addPlayer(player) {
    const twoNames= player.name.toLowerCase().trim()
  .replace(/\s+/g, ' ');
  const twoSurnames= player.surname.toLowerCase().trim()
  .replace(/\s+/g, ' ');

    const isDuplicate = this.players.some(
      (existingPlayer) =>{
       const twoExistingNames=  existingPlayer.name.toLowerCase().trim().replace(/\s+/g, ' ');
       const twoExistingSurnames= existingPlayer.surname.toLowerCase().trim().replace(/\s+/g, ' ');

       return twoExistingNames=== twoNames && twoExistingSurnames === twoSurnames;
   } );
